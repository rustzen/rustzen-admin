...
impl Config {
    ...
    pub fn sqlite_database_path(&self) -> PathBuf {
        if self.sqlite_path == ":memory:" || self.sqlite_path.starts_with("sqlite://") {
            Path::new(&self.sqlite_path).to_path_buf()
        } else {
            let candidate = Path::new(&self.sqlite_path);
            let runtime_root = {
                let root = Path::new(&self.runtime_root);
                if root.is_absolute() {
                    root.to_path_buf()
                } else {
                    std::env::current_dir()
...
mod tests {
    ...
    fn sqlite_path_is_relative_to_runtime_root_when_not_absolute() {
        ...
        assert_eq!(config.sqlite_database_path(), PathBuf::from(".rustzen-admin/data/rustzen.db"));
    }
}
