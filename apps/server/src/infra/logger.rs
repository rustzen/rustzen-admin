...
pub fn init_logging() -> Result<LoggingGuard, Box<dyn std::error::Error>> {
    ...
    let appender = tracing_appender::rolling::daily(&log_dir, &CONFIG.log_file_prefix);
...
fn cleanup_expired_logs() -> Result<(), Box<dyn std::error::Error>> {
    cleanup_logs_in_dir(&CONFIG.log_dir(), &CONFIG.log_file_prefix, Local::now().date_naive())
}
...
mod tests {
    ...
    fn cleanup_keeps_log_files_inside_thirty_day_window() {
        ...
        for name in ["server.2026-06-13", "server.2026-06-14", "unrelated.txt"] {
            std::fs::write(dir.join(name), name).expect("log file");
        }

        cleanup_logs_in_dir(&dir, "server", NaiveDate::from_ymd_opt(2026, 7, 14).expect("date"))
            .expect("cleanup");

        assert!(!dir.join("server.2026-06-13").exists());
        assert!(dir.join("server.2026-06-14").exists());
