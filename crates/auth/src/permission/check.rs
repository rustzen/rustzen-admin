...
impl PermissionsCheck {
    ...
    pub fn check(&self, user: &CurrentUser) -> bool {
        match self {
            Self::Require(code) => user.has_capability(code),
            Self::Any(codes) => codes.iter().any(|code| user.has_capability(code)),
            Self::All(codes) => codes.iter().all(|code| user.has_capability(code)),
        }
    }
}
