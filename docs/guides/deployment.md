...
- `just build` writes versioned deployment outputs under `target/rustzen-admin/`
  and signs the deploy artifacts.
- If no signing key is configured, `just build` creates the default local
  Ed25519 private key at `.rustzen-admin/config/deploy-sign-private.pem`.
- `RUSTZEN_DEPLOY_SIGN_KEY_FILE` or `RUSTZEN_DEPLOY_SIGN_KEY` can override the
  default signing key. CI should use a stable `RUSTZEN_DEPLOY_SIGN_KEY` secret.
...
- `server` uploads are executable binary files with a matching `x86_64` or
  `aarch64` arch. When deploy signatures are required, the binary must include
  an appended signed marker generated by `scripts/deploy-sign.mjs`.
...
- `web` uploads are zip files containing `dist/index.html`, `dist/assets/*.js`
  or `*.css`, and `dist/__rustzen_admin_marker__.json`. When deploy signatures
  are required, the marker must be the signed marker generated by
  `scripts/deploy-sign.mjs`.
...
- Unsigned development web markers have `component` set to `web` and `build_id`
  set to `manual`.
- Signed deploy markers have `schemaVersion`, `component`, `version`, `arch`,
  `contentSha256`, and `signature`.
