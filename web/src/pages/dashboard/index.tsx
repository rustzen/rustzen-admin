...
// 系统健康状态
const HealthCard = () => {
    const { data: health } = useSWR<Dashboard.SystemHealth>(
        dashboardAPI.healthUrl
    );
    const memoryUsage = calculatePercent(
        health?.memoryUsed,
        health?.memoryTotal
    );
            ...
            <div className="flex flex-col gap-5">
                <div>
                    <div className="flex justify-between mb-2">
                        ...
                        <span>
                            {convertUnit(health?.memoryUsed)} /{" "}
                            {convertUnit(health?.memoryTotal)}
                        </span>
                    </div>
                    <Progress
                        percent={memoryUsage}
                ...
                <div>
                    <div className="flex justify-between mb-2">
                        ...
                        <span>
                            {convertUnit(health?.diskUsed)} /{" "}
                            {convertUnit(health?.diskTotal)}
                        </span>
                    </div>
                    <Progress
                        percent={diskUsage}
...
// 性能指标
const MetricsCard = () => {
    const { data: metrics } = useSWR<Dashboard.SystemMetricsData>(
        dashboardAPI.metricsUrl
    );
    return (
        <Card
...
const UserActivityTrendCard = () => {
    const { data } = useSWR<Dashboard.UserActivityChart>(
        dashboardAPI.trendsUrl
    );
    return (
        <div className="grid grid-cols-2 gap-4">
